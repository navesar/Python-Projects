...
def count_down(time):
    mins = int(time // 60)
    secs = int(time % 60)
    mins_str = mins if mins >= 10 else f"0{mins}"
    secs_str = secs if secs >= 10 else f"0{secs}"
    canvas.itemconfig(timer_text, text=f"{mins_str}:{secs_str}")
    if time > 0:
        global timer
...
checkmark_label = tkinter.Label(text="", bg=YELLOW, fg=GREEN)
